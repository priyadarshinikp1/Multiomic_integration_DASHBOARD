...
if not all([genomics, transcriptomics, proteomics]):
    st.warning("Please upload all three omics files to proceed.")
    st.stop()

# --- Load Datasets ---
genomics_data = pd.read_csv(genomics)
transcriptomics_data = pd.read_csv(transcriptomics)
proteomics_data = pd.read_csv(proteomics)
